  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <div className="flex flex-col gap-3 rounded-2xl border border-line bg-surface/90 p-4 shadow-xs">
        ...
        {/* Study Mode Selector - Clean, Balanced 4-Pill Grid */}
        <nav aria-label="문법 4단계 학습 모드" className="w-full rounded-2xl border border-line bg-surface p-1.5 shadow-2xs">
      ...
      {ruleSummary.length > 0 && (
        <details className="group rounded-2xl border border-line bg-surface p-4 shadow-2xs">
          <summary className="flex min-h-6 cursor-pointer list-none items-center justify-between gap-2 text-[14px] font-semibold text-ink">
          ...
          <ol className="mt-3 flex flex-col gap-3 border-t border-line/60 pt-3">
            {ruleSummary.map((rule, i) => (
              <li key={i} className="flex flex-col gap-1">
                <p className={`${fontStyles.korean} font-medium text-ink`}>
                  <span className="mr-1.5 font-mono text-[12px] text-ink-faint">Q{i + 1}</span>
                  {rule.question}
                </p>
                <p className={`${fontStyles.korean} text-ink-soft`}>
