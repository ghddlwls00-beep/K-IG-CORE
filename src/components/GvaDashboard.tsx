  ...
  const chapterGroups = useMemo<ChapterGroup[]>(() => {
    ...
    for (let ch = 1; ch <= 20; ch++) {
      ...
      groups.push({
        ...
        range: `${start}~${end}강`,
        subtopic: first?.chapterSubtopic || `청취 실전 트레이닝 (${start}~${end}강)`,
        enSubtopic: first?.chapterEnSubtopic || "Listening Auditory Mastery",
        description: first?.chapterDesc || "실전 청취 지문을 문맥과 구문별로 체계적으로 풀어주는 직청직해 집중 해설",
        level: isHigh ? "high" : "middle",
        levelLabel: isHigh ? "고등 실전 청취" : "중등 청취 해설",
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3.5 sm:gap-4">
                    {group.lessons.map((lesson) => (
                      <Link
                        ...
                        className="group relative flex flex-col overflow-hidden rounded-xl border border-line bg-raised hover:border-ink/40 hover:shadow-md transition-all duration-200"
                      >
                        ...
                        <div className="flex flex-1 flex-col justify-between p-3 gap-2">
                          ...
                          <div className="flex items-center justify-between border-t border-line/60 pt-2 text-[11px] text-ink-soft font-mono">
                            <span className="text-primary font-medium">실시간 판서</span>
