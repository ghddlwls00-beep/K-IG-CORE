...
export function GvaStreamingPlayer({
  ...
  const [strokesLoaded] = useState(true);
  ...
  const chapterList = useMemo(() => {
    ...
    for (let ch = 1; ch <= 20; ch++) {
      ...
      const subtopic = chLessons[0]?.chapterSubtopic || `영어독해 ${start}~${end}강`;
      list.push({
        chapter: ch,
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 items-start">
          ...
          <div className="lg:col-span-8 flex flex-col gap-3">
            <div className="relative overflow-hidden rounded-2xl border border-line bg-raised shadow-md group">
              ...
              {/* Lecture source indicator */}
              <div className="p-2.5 flex items-center justify-between bg-raised/50 border-t border-line text-[11px] text-ink-soft">
                <span>교재 원문 이미지와 실제 강의 음성을 함께 재생합니다.</span>
                <span className="font-mono text-primary font-semibold">{isPlaying ? "● 강의 재생 중" : "일시정지"}</span>
              </div>
            </div>
              ...
              <div className="flex flex-col gap-1 border-b border-line pb-4">
                <div className="flex items-center justify-between">
                  <span className="font-mono text-[11px] font-bold text-primary tracking-widest uppercase">
                    GVA 독해 직강 · {lesson.chapterLabel || `CH ${Math.ceil(lesson.number / 10)}`}
                  </span>
                ...
                <p className="text-[13px] font-medium text-ink-soft">
                  {lesson.chapterSubtopic || "영어 지문 독해 및 문장 구조 해설"}
                </p>
                {lesson.chapterDesc && (
