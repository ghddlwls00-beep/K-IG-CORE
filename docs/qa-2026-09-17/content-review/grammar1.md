...
**해결:** 질문마다 규칙을 보여 주는 영어 예문 17개(한국어 뜻 포함)를 `sentences` 로 추가 (`apply-gh1-020-theory.cjs`), 화면은 "문법 확인 문제" 전용 — 질문 → [답 보기] → 답 + 영어 예문 🔊, 영작·빈칸·구문 각인·평가 모드 없음, 페이지 음성은 영어 예문을 읽음. **검증** `verify-gh1-020-ui.cjs` 15/15: 8문항, 답 누르기 전 안 보임, 예문 🔊·페이지 재생이 영어 클립 요청(한국어 질문 클립 0), 08강은 기존 영작 화면. 캡처 `out/gh1-020-theory.png`
