...
> **✅ 수정 상태 (2026-09-17, 로컬 수정·검증, 배포 대기):** 아래 G1-01~G1-58 **전부** `scripts/apply-grammar1.cjs` 로 반영 (109파일 267곳, 분할 페이지 포함). 원칙: 틀린·비표준 모범 답안은 교체, 여전히 맞는 옛 문장은 "다른 정답"으로 남겨 감점 방지, 깨진 다른 정답(괄호 합성 비문·메모·부가의문문 없는 평서문)은 교체·삭제. 예외 2건: **G1-47** 은 서로 다른 두 레슨의 각자 문항이라 합칠 것이 없음(변경 없음), **G1-58 gh1-098 #100** `Doesn't he lend some money?` 는 긍정 답을 기대하는 부정 의문문에서 some 이 맞아 유지. 추가로 발견해 같이 고친 것: gh1-059 제목 `[ Page 058]`→`059`, gh1-093 `Page 093]` 잔재, gh1-015 본문의 `혹은 She is beautiful, isn't she?` 문단(G1-10 으로 모범 답안이 되어 삭제). **검증:** `dump-grammar.cjs` GRAMMAR I 기계 검사 전부 0 (물음표 불일치 9→0, 문제·답 수 1→0, 답 없는 문제 2→0, 영어 답안 속 한글 1→0, 분할 페이지 불일치 2→0 등), `scripts/verify-grammar1-fixes.cjs` **85/85** (실제 채점기 `gradeAgainstReferences` 로 표준 답 만점·옛 오답 비만점, 전 답안에 한글·ain't·this 부가의문문 0), 기존 `verify-grammar-grading.cjs` 36/36, 음성 107개 생성·업로드 (버킷 44,874, pending 0).

## 소유자 보고 처리 — 07강 gh1-020 (2026-09-17)

**보고:** "다른 grammar 들이랑 다르게 오디오 재생하면 한국어가 나오고 아래 내용들도 다 한국어". **원인:** 07강은 교재의 "문법 확인 문제" — be동사 문장 규칙을 묻는 한국어 질문 8개와 한국어 답(gh1-021)뿐이고 영어 문장이 하나도 없음. 그런데 화면(`GrammarLearningView.tsx`)이 이를 영작 문제처럼 다뤄 한국어 질문에 영작 칸, **한국어 답을 "영어 정답"**, 음성도 한국어를 읽었고, 문제 페이지와 답 페이지를 차례로 읽어 **16문항**으로 보였음 (운영 캡처 `out/prod-grammar1-gh1-020.png`). 판별식 `lessonKey.includes("020")` 는 GRAMMAR II gh2-020/021 에도 걸림.
**1차 해결 (배포 f04b64b):** 영어 예문 17개 + "문법 확인" 전용 화면. **소유자 재확인:** "여전히 다른 grammar 들이랑 학습법이 달라" → **2차 해결:** 전용 화면을 없애고 다른 GRAMMAR 강의와 똑같은 4단계(영작 훈련 → 빈칸 완성 → 구문 각인 → 종합 평가)로. 문법 질문 8개마다 한국어→영어 영작 문장 3개씩 **24문항** (`apply-gh1-020-composition.cjs`, 예: 나는 소년이다 → I am a boy. / 그들은 네 친구들이 아니니? → Aren't they your friends?), 8개 문답은 단계 위에 접힌 "📘 문법 확인 — be동사 규칙" 요약으로만. **검증** `verify-gh1-020-ui.cjs` 17/17: 08강과 학습 단계 버튼 4개가 글자까지 같음, 24문항·입력칸 24개, 전체 정답 보기에 영어 24문장, 빈칸 완성에 빈칸, 페이지 재생은 영어 클립 요청(한국어 0), 규칙 요약은 처음에 접혀 있고 펼치면 답 8개. 캡처 `out/gh1-020-steps.png`
