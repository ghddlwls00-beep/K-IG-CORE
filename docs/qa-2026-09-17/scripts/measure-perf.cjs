#!/usr/bin/env node
/**
 * Phase 7 — performance, MEASURED on production (no estimates).
 *
...
const FREE = ["/", "/reading", "/student/s1-1", "/phonics/mv1-01", "/grammar1/gh1-006", "/grammar2/gh2-007", "/ld/d001", "/reading/pr001", "/cnn/cnn001"];
const PAID = ["/student/s10-1", "/phonics/hv-48", "/grammar1/gh1-058", "/grammar2/gh2-030", "/ld/d150", "/reading/pr100"];
const CONDITIONS = {
...
(async () => {
  ...
  const browser = await launch({ port: 9371, profile: LICENSED ? path.join(os.tmpdir(), "kig-audit-licensed-profile") : undefined });
  const out = [];
  try {
  ...
  const outFile = path.join(__dirname, `../out/perf${ONLY ? `-${ONLY}` : ""}.json`);
