...
| 항목 | 결정 | 상태 |
|---|---|---|
| ISS-12 이용권 만료 | **첫 등록일 기준 고정** | 로컬 수정·검증 완료 (`verify-license-expiry.cjs` 14/14), 배포 대기 |
| ISS-15 구매처 링크 | 변호사와 상의 후 결정 | DEFERRED — OWNER/LAWYER |
| F-02 원본 녹음 3,187개 | **계속 보관** | 삭제 금지 |
| R-64 노무현·차베스 지문 | **교체** | 완료 (언어·악기 배우기 비교 지문), 음성 44개 생성·업로드, 버킷 44,730, pending 0 |
| R-65 민족별 식사법 지문 | **중립 표현으로 다시 씀** | 완료 (같은 커밋) |
| ISS-00 공개 JS 유출 | 최우선 수정 | 로컬 수정 완료 — 빌드 결과물 유료 문자열 2,403 → 0, 배포 대기 |

## 다음 단계

1. **배포** (만료 수정 + ISS-00 + 지문 2개) → 운영 재측정: `probe-bundle-leak-scope.cjs` READING 0, `probe-entitlement-all.cjs`, READING 스윕
2. L-64 원문 누락 처리 방침
