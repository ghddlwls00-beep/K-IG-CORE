...
| MED-SEC 보안 | 2 / 4 | 기기 초기화 후 verify 불일치(SEC-03), CSP unsafe-inline(SEC-05), 메모리 요청 제한(SEC-07), 토큰 없이 기기 해제(SEC-04) — **✅ SEC-03·04·06 수정, SEC-07 진도 저장 부분 수정 (관리자 로그인 제한은 소유자 지시로 보고만), ✅ SEC-05 nonce CSP (소유자 결정 "진행하자", 로컬 검증 80/80·유료 10/10, 배포 대기 — 모든 페이지가 요청마다 서버 렌더링됨)** | `phase8-security.md` |
