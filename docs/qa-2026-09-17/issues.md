...
### ISS-01 GRAMMAR I 부가의문문 모범 답안이 틀림 (this→it, these→they, ain't I, 주어 불일치) — ✅ 로컬 수정·검증, 배포 `d6da0a4` (`apply-grammar1.cjs`, `verify-grammar1-fixes.cjs` 85/85)
...
### ISS-02 GRAMMAR I 문제-답안 데이터 손상 (어긋남·합쳐짐·메모 혼입) — ✅ 로컬 수정·검증, 배포 `d6da0a4` (G1-20·25·33·42, 같은 스크립트)
...
### ISS-03 GRAMMAR I be동사 뜻을 "하다" 로 가르침 (G1-16) — ✅ 로컬 수정, 배포 `d6da0a4` (문제·답안 모두 `--하다` 삭제)
...
### ISS-04 GRAMMAR II 모범 답안의 문법 오류와 잘린 문장 — ✅ 로컬 수정·검증, 배포 `619003e` (`apply-grammar2.cjs`, `verify-grammar2-fixes.cjs` 76/76)
...
### ISS-05 STUDENT 사실·문법 오류 (광복 연도, 분단 원인, 한글 창제 연도, 비문) — ✅ 로컬 수정·검증, 배포 `f3f6cb6` (`apply-student.cjs`, `verify-student-fixes.cjs` 44/44)
...
### ISS-06 STUDENT s19-3 레슨에 받아쓰기 문장이 하나도 없음 (S-05) — ✅ 수정·배포·운영 확인 (de4cbcf)
- **운영 확인 (2026-09-17, 배포 후, LIFE 이용권 프로필 `check-prod-pages.cjs`):** `/student/s19-3` 영어 4문장·한국어 4문장 표시, 탭 딕테이션 0/4. **원본 녹음 대조:** `stt-student.cjs` 로 s19-3 녹음 4개(전체 28초 + 문장 2·3·4)를 받아 적은 결과가 레슨 영어와 단어까지 일치 (형제 레슨 s19-2 는 문장·청크별 15개 녹음, 3문장)
...
### ISS-07 STUDENT 청크 연습이 다른 레슨 문장을 연습시킴·한영 뒤섞임 (S-06, S-07) — ✅ 로컬 수정·검증, 배포 `f3f6cb6` (82개 레슨 청크 1,050/1,050 자기 문장 안)
...
### ISS-08 LISTENING d109 받아쓰기 한 칸에 다음 회차 본문 10문장이 통째로 붙음 (L-42) — ✅ 로컬 수정·검증, 배포 `1adc2f5` (`apply-listening.cjs`, `verify-listening-fixes.cjs` 100/100)
- **분류** 데이터 손상 · **심각도** High · **접근** 유료
...
### ISS-09 READING 지문 데이터 손상 (주석·보기·발문 혼입, 순서 뒤섞임, 잘림) + 뜻 없는 문장 — ✅ 로컬 수정·검증, 배포 `1a696f3` (`apply-reading-passages-*.cjs`, `verify-reading-fixes.cjs` 110/110)
...
### ISS-10 READING 어휘 카드: 문맥과 다른 뜻 213장, 뜻 없는 카드 69장, 스크립트 페이지 옛 뜻 510장, 품사 모순 1,020장+ — ✅ 로컬 수정·검증, 배포 `1a696f3` (`apply-reading-cards-*.cjs`, `verify-reading-fixes.cjs` 110/110)
...
### ISS-13 iPhone Safari 7일 미사용 시 이용권·기기 ID 삭제 → 재등록마다 기기 칸 소모 (SEC-02, LX-02) — ✅ 로컬 수정·검증, 배포 `e4cc125`
...
### ISS-14 관리자 목록이 발급한 브라우저의 기록만 보여 줌, 검색 없음 (ADM-01) — ✅ 로컬 수정·검증, 배포 `25f8c1d`
...
### ISS-16 다크 모드에서 재생·주요 버튼의 글자·아이콘이 거의 안 보임 (UX-01) — ✅ 로컬 수정·검증, 배포 `a5a1537`
...
| MED-L LISTENING | 56 / 38 | 전사 오류(Lite·Wilbur·Iditarod 등), 숫자 표기, 낡은 사실(L-27·49), **불쾌 표현**(L-74·84 freaks·minstrel·primitive tribes), 힌트-답 이름 불일치(L-00a) — **✅ L-74·L-84 외 전부 로컬 수정·검증, 배포 `1adc2f5`** (`listening.md` "처리 결과"; L-74·L-84 는 소유자 결정 대기) | `content-review/listening.md`, `listening-status.md` (PASS 64/FAIL 212) |
| MED-R READING 지문 | 64 / 32 | 영한 정렬 어긋남 다수, 오역(뜻 반대), 낡은 전망(2010·2025), 정치·고정관념 지문(R-64 노무현·차베스, R-65 식사법), 중복 지문 9쌍(R-01 — **소유자 결정으로 한쪽씩 새 지문으로 교체**), 사실(재향군인병·마라톤·Purloined Letter·안락사법) — **✅ 소유자/변호사 대기 항목(R-01 한쪽 교체·R-30 pr054·R-32 pr078·저작권) 외 전부 로컬 수정·검증, 배포 `1a696f3` · R-01 `f04b64b`** (`reading.md` "처리 결과") | `content-review/reading.md` |
| MED-RV READING 어휘 | (ISS-10 에 포함) | ✅ 3,584장 전부 재작성·검증, 배포 `1a696f3` | `content-review/reading.md` §어휘 카드 |
| MED-V VOCA | 6 / 6 | mv2-12 달 이름 반복(V-03), 같은 뜻 짝 드릴 채점(V-04), 철자 오류 6(V-05), `apparently` 분명히(V-06), 다의어 첫 뜻 누락(V-07) — **✅ V-03~V-11 로컬 수정·검증, 배포 `fa6c273`** (`voca.md` "처리 결과"; 수정 중 발견한 라이트너 오답노트 옛 뜻 고정 문제 포함) | `content-review/voca.md`, `voca-status.md` (PASS 102 · NOTE 42 · FAIL 51) |
| MED-ADM 관리자 | 3 / 3 | 테스트 등록이 고객 칸 차지(ADM-02), 한도·해금 드롭다운 즉시 저장(ADM-03·04), 81강 표기(ADM-05) — **✅ ADM-02~07 전부 로컬 수정, 배포 `25f8c1d` · ADM-07 `de4cbcf`** (ADM-07 새로고침 때 바뀐 기록만 내려받기) | `phase2-admin.md` |
| MED-SEC 보안 | 2 / 4 | 기기 초기화 후 verify 불일치(SEC-03), CSP unsafe-inline(SEC-05), 메모리 요청 제한(SEC-07), 토큰 없이 기기 해제(SEC-04) — **✅ SEC-03·04·06 수정, SEC-07 진도 저장 부분 수정 (관리자 로그인 제한은 소유자 지시로 보고만), ✅ SEC-05 nonce CSP (소유자 결정 "진행하자", 로컬 80/80·유료 10/10, **배포 `9706a83`·운영 69/69** — 모든 페이지가 요청마다 서버 렌더링되어 홈·목록·무료 레슨 응답 0.3~0.5초, 원인 PERF-04 함수 지역)** | `phase8-security.md` |
| MED-UX 접근성 | 1 / 4 | 음성 플레이어 문장 점 버튼 1~5px 누를 수 없음(UX-05), 금색 작은 글자(UX-02), `🔁` 이름(UX-03), 17~21px 칩(UX-06) — **✅ UX-02·03·05·06 로컬 수정·측정, 배포 `de4cbcf`** (24px 미만 운영 483 → 0, 라이트 대비 실패 70 → 2·다크 32 → 2 남은 것 CNN; 다크 금색 버튼 흰 글자·LISTENING 받아쓰기 점도 함께). UX-04 는 CNN(폐지)이라 기록만 | `phase6-ux-a11y.md` |
