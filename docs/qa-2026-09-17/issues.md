...
### ISS-00 READING 유료 지문 254개 전체(영어·한국어·어휘 카드)가 누구나 받는 공개 JS 파일에 들어 있음 — ✅ 수정·배포·운영 확인 (886b08c)

> **수정 (2026-09-17):** `readingUtils.ts` 의 중앙 사본 import 와 조회 함수 2개 삭제, `ReadingLearningView.tsx` 대체 조회 삭제 (512개 레슨 파일 모두 자기 데이터 보유 확인), 꺼진 자동 퀴즈 코드 안의 pr081 한국어 문장 직접 인용 제거. **검증:** 새 스크립트 `scripts/scan-build-static.cjs` — 빌드 결과물 `.next/static` 전체에서 유료 문자열 6,263개 검색: 수정 전 빌드 **2,403건** → 수정 후 **0건**. **운영 재측정 (배포 886b08c, 2026-09-17 10:40 KST, 쿠키 없음):** `probe-bundle-leak-scope.cjs` — `/reading/pr001` 이 받는 공개 JS 1,601 KB → 725 KB, 유료 254지문 영어 **0/1,423**·한국어 **0/1,423** (수정 전 1,404·1,401), 다른 과정·검색 색인 0. 카드는 느슨한 기준(단어와 뜻이 JS 어디에든 따로 있음)이 3,171 → 625 인데 남은 625는 "tree/나무" 같은 흔한 단어가 무료 VOCA 사전·화면 문구에 있어서임 — 카드 레코드 형태(`word…lemma…`) 검색 `probe-reading-cards-strict.cjs` **0/3,556**, 카드 설명 문구 없음. `probe-entitlement-all.cjs` 1,748/1,748.
...
### ISS-01 GRAMMAR I 부가의문문 모범 답안이 틀림 (this→it, these→they, ain't I, 주어 불일치) — ✅ 로컬 수정·검증, 배포 대기 (`apply-grammar1.cjs`, `verify-grammar1-fixes.cjs` 85/85)
...
### ISS-02 GRAMMAR I 문제-답안 데이터 손상 (어긋남·합쳐짐·메모 혼입) — ✅ 로컬 수정·검증, 배포 대기 (G1-20·25·33·42, 같은 스크립트)
...
### ISS-03 GRAMMAR I be동사 뜻을 "하다" 로 가르침 (G1-16) — ✅ 로컬 수정, 배포 대기 (문제·답안 모두 `--하다` 삭제)
...
### ISS-04 GRAMMAR II 모범 답안의 문법 오류와 잘린 문장 — ✅ 로컬 수정·검증, 배포 대기 (`apply-grammar2.cjs`, `verify-grammar2-fixes.cjs` 76/76)
...
### ISS-05 STUDENT 사실·문법 오류 (광복 연도, 분단 원인, 한글 창제 연도, 비문) — ✅ 로컬 수정·검증, 배포 대기 (`apply-student.cjs`, `verify-student-fixes.cjs` 44/44)
...
### ISS-06 STUDENT s19-3 레슨에 받아쓰기 문장이 하나도 없음 (S-05) — ✅ 로컬 수정·검증, 배포 대기 (영어 4문장·청크 복원)
...
### ISS-07 STUDENT 청크 연습이 다른 레슨 문장을 연습시킴·한영 뒤섞임 (S-06, S-07) — ✅ 로컬 수정·검증, 배포 대기 (82개 레슨 청크 1,050/1,050 자기 문장 안)
...
### ISS-11 VOCA 퀴즈·드릴이 `March`·`May`·`Miss` 를 다른 단어 뜻으로 채점, 대문자 단어 50칸은 퀴즈에서 빠짐 — ❌ 철회 (감사 오류)

> **정정 (2026-09-17, 수정 착수 전 재확인):** 이 이슈는 없음. 감사는 `vocaUtils.ts` 의 소문자 조회만 보고 판단했으나, 컴포넌트가 퀴즈·드릴에 넘기는 것은 사전이 아니라 `PhonicsLearningView.tsx:211-218` 에서 **카드와 같은 조회(`getMeaning`, 원형 우선)로 만든 표**임 (감사 대상 `c14c03d` 에도 동일). 증거: 실제 코드(`getVocaDictionaryForWords` + `generateActiveRecallQuizzes`/`generateSpeedDrillItems`)로 195레슨 5,831단어 — 카드≠퀴즈 0, 퀴즈 누락 0, 드릴 불일치 0, 한→영 같은 뜻 보기 0 (`scripts/verify-voca-quiz-meanings.cjs`). 감사 자신의 운영 캡처 `out/rendered-v2/phonics/mv2-12.json` 도 `QUESTION #1 / 30`, `November` 보기에 `11월`. `dump-voca.cjs`·`status-voca.cjs` 가 전체 사전을 소문자로 찾는 모형이라 틀린 수치(7건·50칸)를 냈음 — `status-voca.cjs` 에서 V-01·V-02 제거. **High 1건 감소.**
...
### ISS-12 기간제 이용권이 코드를 다시 입력하면 기간이 새로 시작 (SEC-01) —✅ 수정·배포 (822ec46, 886b08c 와 함께). 운영에서 기간제 코드로는 시험 못 함 (코드 입력 금지) — 검증은 로컬 스크립트 14/14
...
### ISS-14 관리자 목록이 발급한 브라우저의 기록만 보여 줌, 검색 없음 (ADM-01) — ✅ 로컬 수정·검증, 배포 대기

> **수정 (2026-09-17):** 목록을 서버 기록(`/api/license/status`)으로 구성 — 어느 브라우저에서나 전체 이용권·건수·차단 수. 검색(코드 일부·하이픈 없이, 메모, 기기 이름, 이용권 종류, 차단 사유). 발급일·메모를 서버 암호화 기록에 저장(`createIssuedLicenseRecord`), 메모 추가·수정 버튼, 새 API `/api/admin/record-meta`(관리자 세션 필수, **기존 기록만** 수정 — 임의 코드로 기록을 만들 수 없음). 예전 발급 브라우저의 localStorage 기록은 읽기만 하고 "서버로 옮기기"(서버에 없는 메모·발급일만 채움) 후 삭제. 같이: ADM-02 고객 코드에 관리자 브라우저를 등록하던 테스트 버튼 삭제(SEC-01 이후엔 기간제 기간까지 시작시킴), ADM-03 기기 한도·ADM-04 STUDENT 해금 드롭다운은 확인 창 후 저장, ADM-05 `81강` 표기 제거(완료 수 분모는 서버 챕터 데이터에서 계산), ADM-06 새 메모를 브라우저에 쓰지 않음. ADM-07(새로고침마다 전 기록 조회)은 남음. **검증:** `scripts/verify-admin-records.cjs` 37/37 (실제 route·저장소 코드, 임시 로컬 저장소·임의 비밀값), `scripts/verify-admin-ui.cjs` 25/25 (격리된 `next dev` — 운영 저장소 설정 공백·임의 비밀값·PIN 입력 없음 — 에서 화면 렌더링·검색·확인 창 취소/저장·옛 기록 옮기기·기록 없는 새 브라우저에서 4건 표시, 콘솔 오류 0, 캡처 `out/admin-ui.png`), `verify-license-expiry.cjs` 14/14, `tsc` 0, `pnpm build` 성공.
> **배포 후 소유자 할 일 (1분):** 예전에 코드를 발급하던 PC 브라우저로 `/admin/license` 에 들어가 노란 안내의 **[서버로 옮기기]** 를 한 번 누르면, 그 브라우저에만 있던 메모·발급일이 서버로 옮겨짐. 누르기 전에도 목록 자체는 모든 브라우저에서 보임 (메모·발급일만 "발급일 기록 없음"). 다른 PC·휴대폰에서 관리자 로그인 → 전체 목록이 보이는지 확인.
