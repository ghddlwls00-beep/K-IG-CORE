 ...
 * 예외 한 줄 = { lesson, kind(고침 · 바꿈 · 더함 · 빠짐 · 차례 · 짝 · 원본 없음), text, original?, why } — kind · text 가 글자까지 같아야 맞음.
...
import { compareLesson, sentencesOf, koParasOf, key } from "./lib/student-original.mjs";
...
let total = 0, same = 0, explained = 0; const bad = [], used = new Set(), counts = {}, altCopy = [];
for (const id of ids) {
  const { en, ko } = data[id];
  total += en.length;
...
console.log(`STUDENT ${ids.length}과 · 영어 문장 ${total} · 원본과 글자까지 같음 ${same} · 원본과 다른 곳 ${Object.values(counts).reduce((a, b) => a + b, 0)} (${Object.entries(counts).map(([k, v]) => `${k} ${v}`).join(" · ")})`);
