 ...
 *   ④ 고치기 전(HEAD)과 빈칸이 달라진 문항 수 · 그 가운데 n't 가 없는 문항(있으면 안 됨 — 바꾼 범위 확인)
 *
 *   node check-grammar-cloze.cjs [--old] [--list]
 *   --old : HEAD 의 GrammarLearningView.tsx 로 ①②③ 을 셈(대조군 — ① 이 0 이 아니어야 정상, exit 1)
 * ① 이나 ③ 이 있으면 exit 1.
 */
const fs = require("fs");
...
const headSrc = execSync("git show HEAD:src/components/GrammarLearningView.tsx", { cwd: REPO, encoding: "utf8", maxBuffer: 1 << 26 });
...
console.log(`${OLD ? "[HEAD] " : ""}GRAMMAR 모범 답안(서로 다른) ${n} · ① 반쪽 빈칸 ${half} · ② n't 가 있는데 빈칸은 딴 곳 ${visible} · ③ 온전한 꼴이 exact 아닌 빈칸 ${notExact}${OLD ? "" : ` · ④ HEAD 와 빈칸이 달라진 문항 ${changed} (n't 없는 문항 ${changedNoNeg})`}`);
