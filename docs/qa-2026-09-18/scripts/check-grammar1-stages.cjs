 ...
 *   --old : HEAD 의 curriculumPresentation.ts 로 (고치기 전 — 경계 5곳의 영어 답 쪽이 다음 단계로 나와 0 이 아니어야 정상, exit 1)
 */
const fs = require("fs");
...
const src = OLD ? execSync("git show HEAD:src/lib/curriculumPresentation.ts", { cwd: REPO, encoding: "utf8" }) : fs.readFileSync(path.join(REPO, "src/lib/curriculumPresentation.ts"), "utf8");
...
console.log(`${OLD ? "[HEAD] " : ""}grammar1 페이지 ${n} · 부제목 단계가 목록 단계와 다른 페이지 ${bad.length}`);
