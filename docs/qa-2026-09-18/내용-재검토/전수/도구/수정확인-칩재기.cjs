 ...
 *  → 수정확인/고칠-칩.json(줄 · 바뀐 문장 main 칩 → 고친 뒤 칩 · 번짐 수) · 하나라도 어긋나면 exit 1.
 *   node 수정확인-칩재기.cjs
 */
const fs = require("fs");
...
function measure(id, line, want = [], gone = []) {
  const rows = sc[id] || [], main = mainHint(id), mc = NEW(main), pc = NEW(line), bad = [], changed = [], newFlags = [];
...
{
  const same = measure("d177", mainHint("d177"), [[4, "Rudd"]]);
  const ten = measure("d177", "Rudd. destination. amused. sent. Ten minutes later. astonished", [[4, "Rudd"]]);
  const ok = measure("d177", "Rudd. destination. amused. sent. minutes later. astonished", [[4, "Rudd"]], [[4, "destination"]]);
...
if (process.argv.includes("--applied")) {
  ...
  const off = done.filter((x) => mainHint(x.강).trim() !== x["고칠 힌트 줄"].trim());
