...
> **이어서(최종 관문 2026-09-25)**: 관문 15 재검토가 힌트 칩을 다시 봄 — ① 칩 글(힌트 줄) 고침은 관문 15 첫 배포(`1ef7d36`) · ② 칩이 그 낱말이 없는 문장에도 뜨는 '번짐' 86곳은
> 칩 규칙(`LdLearningView` pickHintsFor — 낱말 머리에서만 맞음 · 대문자 낱말은 대문자 그대로 · 수가 든 칩은 그 수가 모두 있을 때만, `f2f21f2`)으로 고침 —
> 소유자 결정(2026-09-25 10:5x) · 두 번째 배포 `bf3f4b6`. 필요한 칩 잃음 0 · 안전장치(강의 칩 전부)가 새로 켜진 행 4 · 앱과 검사 복제 칩이 다른 행 0(2,217행).
> 배포 뒤 LISTENING 전부 × 3화면을 스윕이 다시 봄(관문 0).
...
### BUG-031 · 영어 문장 속 로마자 한국어 낱말(경주 · 불국사 · 추석 · 서울 …)을 영어식으로 읽음 — 56문장 → **다시 고침 · 배포 `dd12758`(2026-09-26 03:19) · 운영 확인 · 사장님이 56문장을 들어 보심**

> **이어서(2026-09-26)**: 첫 고침 `397f1e8`(아래 표)은 영어 목소리가 그 낱말만 한국어 모드(`<lang xml:lang="ko-KR">`)로 바꿔 읽는 방식이었고, 운영 확인은 '새 소리 파일을 불렀다' 까지였다 —
> 소리를 들은 사람이 없었다. 사장님이 들어 보고 **"경주 제대로 수정 안됬네 엉망인데"**(02:3x). → 같은 두 문장(s20-4 #1 · #2)을 다섯 방식으로 들려 드림(지금 운영 · 고치기 전 · 한국어 성우 ·
> 발음 기호 · 철자 바꿈) → 사장님 **"라가 좋네"**: 같은 영어 목소리가 끊김 없이 읽고 낱말에만 발음 기호(`Gyeongju ⟨ˈkjʌŋˌdʒu⟩` — VOCA 발음 둘 낱말과 같은 꼴, 생성기가
> `<phoneme alphabet="ipa">` 로). `src/lib/lessonSpeechForm.ts` 표를 낱말 → 한글에서 낱말 → 발음 기호로 · 생성기가 문장 속 기호를 읽게 · 56문장 다시 만들어 **사장님이 모두 들어 보신 뒤 "올려"**.
> 검증: compare-shared-code `--base 397f1e8` 허용 목록(`관문15-고침/소리꼴-발음기호-허용.json`, 비교 전에 적음 — 42쪽 · 글 73)과 꼭 같음 · 깨기 넷 exit 1 · 새 클립 56/56 조각 온전 · R2 56 ·
> **운영(03:19 올라온 뒤, 이용권)**: 그 문장들의 버튼 299 = 재생 249 · 녹음 50 → **재생 249/249 소리 남 · 새 클립 재생 236 · 옛 한글판 요청 0** · 로그인 없이 무료 s1-2 새 키 `26-081f831b4d5ea0cc` 206 · 한글판 키 403.
> 42쪽 × 3화면 전체 다시 돌기는 하지 않음(바뀐 것이 소리 글뿐임을 도구로 확인 — 보고서 J절 15).
...
| **1분 확인(사장님)** | `/student/s20-4` 첫 문장 🔊 → 같은 목소리가 끊김 없이 'Gyeongju' 를 '경주' 에 가깝게 읽으면 고쳐진 것(2026-09-26 방식) |
...
| ~~BUG-034~~ **고침 · 배포 `29d527a`(2026-09-26 21:05 푸시 · 21:06:57 올라옴 — 사장님 "올려") · 운영 확인**: 틈에서 정지 **3/3 멈춤** · 틈에서 일시정지 **3/3 멈춤 → 다시 재생 3/3 둘째 문장** · 가운데 6/6 그대로(`bug034-deployed-*.txt`) · 빠른 동작 6과정 3번(9 · 9 · 8 — 8 인 판은 gh2-007 '다시 재생' 에 소리 오류 2 · 소리는 남 · 모두 9번 중 1번 — findings-log OBS-0926-G2-NOTSUPPORTED). 아래는 배포 전 기록: **고침 — 로컬 확인(2026-09-26 13:0x · 사장님 "이것들 너가 그냥 해").** `src/lib/speech.ts` 한 곳: 대기열이 이어지면 문장 사이 틈에도 '말하는 중'(정지 켜짐) · 틈에서 일시정지하면 다음 문장을 붙잡아 두었다가 다시 재생에서 그 문장부터 · 새 대기열(다음/이전 문장 · 처음부터)은 그대로 재생. 로컬 운영 빌드: 틈에서 정지 **3/3 멈춤**(고치기 전 운영 0/3) · 틈에서 일시정지 **3/3 멈춤 → 다시 재생 3/3 둘째 문장**(고치기 전 첫 문장부터) · 가운데 3/3 그대로 · 연타 뒤 정지 → 재생 '세 번 모두 새 소리'(고치기 전 운영 4/4 실패) · 빠른 동작 6과정 9/9 · 새 클립 0(생성기 pending 0) — `공통기능-다시-0926/results/bug034-fixed-local-*.txt` · `common-0926-local-fix.jsonl`. 아래는 찾았을 때의 기록: **2026-09-26 새벽 '공통 기능 다시'(D 빠른 동작)에서 찾음 · P3(출시를 막지 않음)** | **위 플레이어: 문장과 문장 사이 약 0.3초 동안 '정지' 는 꺼져 있어 눌러도 다음 문장이 그대로 나오고, '재생/일시정지' 는 멈추지 않고 첫 문장부터 다시 시작.** 문장 가운데서 누르면 둘 다 제대로. READING pr001 에서 정지 · 일시정지 각 3/3 되풀이(가운데 3/3 정상). 문장 2 ~ 3초에 틈 0.3초 — 누름 열 번에 한 번꼴. 같은 위 플레이어(말하기 대기열)를 쓰는 다른 과정도 코드상 같음(이번엔 READING 만 확인) | `src/lib/speech.ts:685-698`(finishRun 이 문장마다 speaking:false) · `:1172-1181`(다음 문장까지 gap 틈) · `src/components/AudioPlayer.tsx:200-201`(단추 모양 · 정지 켜짐 = speech.speaking) · `:103-107`(speaking 이 아니면 startQueue(0) — 처음부터) | `공통기능-다시-0926/results/bug034-stop-in-gap.txt` · `bug034-toggle-in-gap.txt` · `bug034-timeline.txt`(시각별 소리 사건) · 재현 `scripts/diag-reading-gap-stop-0926.cjs [--toggle]` | 대기열이 이어지는 동안(문장 사이 틈 포함)은 '말하는 중' 으로 둠 — 틈에도 정지가 켜져 대기열을 끝내고, 일시정지는 대기열을 멈춤. 고친 뒤 같은 재현으로 틈 3/3 · 가운데 3/3 |
| ~~BUG-035~~ **고침 · 배포 `29d527a`(2026-09-26 21:06:57) · 운영 확인**: /student 잠금 표시가 text-ink-soft(배포 감지 스크립트가 운영 HTML 에서 확인) · check-a11y 운영 /student 밝은 테마 대비 낮은 글 **19 → 0**(`a11y-compare-deployed.txt`). 아래는 배포 전 기록: **고침 — 로컬 확인(2026-09-26 13:0x).** `CourseDashboard.tsx:464` 잠금 표시 글자를 옆 '접기 ▲' 와 같은 `text-ink-soft`(#4E4B46)로 — 로컬 check-a11y: /student 밝은 테마 대비 낮은 글 **19 → 0** · 나머지 값 그대로(`a11y-compare-localfix.txt`). 아래는 찾았을 때의 기록: **2026-09-26 새벽 '공통 기능 다시'(접근성)에서 찾음 · P3** | STUDENT 과정 목록(밝은 테마)의 잠긴 장 표시 **'🔒 잠금' 글자 대비 4.47:1** — WCAG AA 기준 4.5:1 에 조금 못 미침(19개). 앞 장을 아직 안 끝낸 학습자 · 이용권 없는 사람에게 보임(9/18 은 이 표시가 안 보이는 상태로 재어 안 나옴) | `src/components/CourseDashboard.tsx:466`(장 상태 표시) — 글자 rgb(114,109,102) · 바탕 rgb(242,239,233) | `공통기능-다시-0926/results/a11y-0926.json` · `a11y-compare-0918-vs-0926.txt` | 잠금 표시 글자색을 조금 진하게(대비 4.5 이상) |
...
- LISTENING L-74 / L-84, READING pr054 (R-30) · pr078 (R-32)
  → **지금 상태(2026-09-26 기록을 따라가 확인 — `최종관문-작업기록.md` '9/17 결정 대기 추적')**: L-84 다섯 곳(d232 · d234 · d255 · d256 · d257)과 L-80 d215 는
  결정 · 고침 · 배포됨. L-74 는 d196 만 결정표 7(알림)로 정해짐. **결정 기록이 없는 것 셋 — R-30 pr054 · R-32 pr078 · L-74 d194 n6('freaks', 한국어 오역만 고침)** — 셋 다 지금 운영에 그대로.
  → **사장님 결정 '다 그대로'(2026-09-26 04:0x, 이 채팅 "그냥 다 그대로 둬")** — 셋 다 지금 글 그대로 · 이 줄의 학습 내용 쪽은 닫음.
