...
| KIG-013 (P2) | (아래 커밋) | 잠긴 레슨의 완료/북마크 토글. **레슨 페이지 쪽은 KIG-001 로 이미 해소됨** (early return 이 `LessonPaywall` 만 렌더). 남아 있던 대시보드 카드의 별 버튼에 `disabled={!isUnlocked}` 추가 | 레슨 페이지는 운영에서 확인: 잠긴 레슨 버튼 문구 **0회** vs 무료 레슨 **1회**. ⚠️ 대시보드 별 버튼은 **UI 검증 보류** (아래 §4 참조) |
...
> ### ⛔ 이미 완료된 이슈 — 재작업 금지
> `KIG-001`(984a553) · `KIG-002`(37d2a26) · `KIG-011`(2f8a97e) · `KIG-018`(c50a9ea) · `KIG-024`(efe57da) · `KIG-013`(e83b8ae)
> 위 6건은 §1 완료 표에 검증 결과까지 기록되어 있습니다. 다시 손대지 마세요.

### 남은 이슈 (35건)

1. **그룹 A — 코드 소규모 (12건, 다음 착수 대상)**
   `KIG-019`(VOCA 퀴즈 복수정답 오채점) · `KIG-031`(레슨 하단 원본 경로 노출) · `KIG-032`("음절 수"가 실제로 글자 수) · `KIG-033`("마스터 체크" 3번 눌러야 함) · `KIG-034`(어휘 카드 번호 `#010`) · `KIG-035`(속독 WPM 상한 없음) · `KIG-036`(모바일/데스크톱 탭 라벨 잘림)
   §4 미분류: favicon 404 · TTS 비공식 API(`translate.google.com`) · 섹션 소개 레슨 수 이중 집계 · Step 1 "전체 정답 보기"가 Step 2 빈칸까지 공개 · LISTENING Step 1 정답 공개 후 선택 변경 가능
...
`/[course]` 목록 페이지의 레슨 카드가 headless Edge 에서 **DOM 에 생성되지 않습니다.** (`<li>` 0개, 본문 텍스트 437자, JS 오류 없음. dev·운영 양쪽 동일)
섹션은 기본 접힘이고 클릭하면 `aria-expanded` 는 `true` 로 바뀌지만(`{isOpen && ...}` 블록은 통과) 카드가 나오지 않습니다.
→ **대시보드 UI(별 버튼, 완료 표시, 카드 레이아웃)는 이 환경에서 검증 불가.** 이 영역을 건드리는 이슈는 사람이 직접 확인해야 합니다.

### ⚠️ dev 서버가 모든 라우트에 404 를 반환할 때
`.next` 캐시가 꼬이면 `/` 를 포함한 **모든 경로가 404** 가 됩니다 (서버는 정상 기동하고 로그도 정상).
→ `rm -rf .next` 후 dev 서버 재기동으로 복구됩니다. (`dangerouslyDisableSandbox` 필요, 435MB)
