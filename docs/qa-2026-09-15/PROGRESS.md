...
| KIG-015 (연음 음성 3,403개) | ~~Azure Speech 키~~ → **키 확보됨.** 남은 것은 `generate-azure-ava.mjs`가 `generateLiaisonPoints()` 산출 문구를 수집하지 않는 문제 |

> **교재 원본은 Cloudflare에 없습니다 (2026-09-15 확인).** `k-iglab` 버킷 전체를 열거한 결과
> `audio/`, `video/`, `private/license-records/`(이용권 JSON 2개)뿐입니다. 소유자는 업로드한 것으로
> 기억하고 있었으나 실제로는 없습니다. KIG-004·007·008은 **다른 보관처를 찾을 때까지 착수 불가**입니다.

> **Azure Speech는 연결 완료 (2026-09-15)**. `.env.local`에 `AZURE_SPEECH_KEY`/`AZURE_SPEECH_REGION=koreacentral`,
> F0 무료 티어(월 50만 자). 콘텐츠 클립 **1,158개를 생성·업로드**해 결측 0이 되었고,
> KIG-002가 깨뜨렸던 READING 커버리지가 40.6% → **100%** 로 복구됐습니다(사이트 경유 실측).
> 남은 결측은 연음 클리닉(KIG-015)뿐입니다.
