...
> 최종 갱신: 2026-09-16 (10차 — RE-004 · KIG-008 · RE-008 완료. 9차는 RE-006 CSP `8c59d82`)
...
## 1-G. 7차 세션 완료분 (2026-09-16) — `NEXT-SESSION.md` §B 대기열 1~2번

| 이슈 | 커밋 | 내용 | 검증 |
|---|---|---|---|
| RE-014 | `bb4baf2` | 페이지당 h1 정확히 1개. 홈은 캐러셀이라 슬라이드 제목이 전부 h2 + 헤더 브랜드가 `div` → **h1 0개**였고, `/ld/d001`·`/ld/d001-1` 은 레슨 제목 h1 + `LdLearningView` 내부 h1 으로 **2개**였습니다 | 운영 수정 전 **20/23 exit 1** → 로컬 빌드 **23/23 exit 0** → 배포 후 운영 **23/23 exit 0** |
| RE-016 | `3705348` | `not-found.tsx` · `error.tsx` · `global-error.tsx` 추가. **`loading.tsx` 는 의도적 제외**(아래) | 운영 수정 전 **5/5 FAIL exit 1** → 로컬 **5/5 PASS exit 0** → 배포 후 운영 **5/5 PASS exit 0** |

### RE-014 상세

- **문서 불일치를 실측으로 판정**: `STATUS-2026-09-16.md` 는 `/ld/d001` h1 을 **3개**로, `NEXT-SESSION.md` 는 **2개**로 적었습니다.
  운영 실측 결과 **2개**이며 NEXT-SESSION 이 맞았습니다 —
  `["001회 · 실전 듣기 평가", "5단계 음향 인지 리스닝 마스터리"]`.
- 홈: 헤더 브랜드 `div` → `h1`. **시각 변화 0 을 CSS 로 확인**했습니다(가정하지 않았습니다):
  `h1,h2,h3,h4,h5,h6{font-size:inherit;font-weight:inherit}` +
  `*,:after,:before,::backdrop{box-sizing:border-box;border:0 solid;margin:0;padding:0}`.
  클래스와 인라인 `font-family` 는 그대로 두었습니다.
- `DialogueLearningView` 도 같은 형태(챕터 개요 h1)였습니다. `man`·`woman`·`adults*` 전용인데
  **그 과정들은 코스 레지스트리에 없어 라우트 미노출**이라 현재 도달 불가입니다. 재노출 시 재발하지 않도록 함께 내렸습니다.
- **미수정·보고만**: `/admin/license` 는 SSR 시 h1 이 0개입니다(`isCheckingAuth` 초기값 `true` → "관리자 인증 확인 중..."만 렌더).
  하이드레이션 후에는 정확히 1개입니다. 내부 라우트의 일시적 클라이언트 상태라 게이트에서 제외하고 사유를 주석에 남겼습니다
  (영구히 빨간 게이트는 게이트가 없는 것과 같습니다).

### 🔴 RE-016 — `loading.tsx` 를 넣으면 안 됩니다 (실측)

감사는 4개 파일을 요청했지만 **`loading.tsx` 는 넣을 수 없습니다.** 루트 `loading.tsx` 는 모든 라우트를
Suspense 경계로 감싸고, 그 결과 Next 가 **404 를 알아채기 전에 200 을 flush** 합니다.
같은 빌드·같은 서버에서 `loading.tsx` 만 넣고 뺀 비교 측정입니다:

| 경로 | `loading.tsx` 있음 | 없음 |
|---|---|---|
| `/x` (→ `[course]` 매칭 후 `notFound()`) | **200** (소프트 404) | 404 |
| `/x/y` (→ `[course]/[lesson]`) | **200** (소프트 404) | 404 |
| `/x/y/z` (어느 라우트도 미매칭) | 404 | 404 |
| `/ld/x` | **200** (소프트 404) | 404 |
| `/t/x` | **200** (소프트 404) | 404 |

소프트 404 는 기본 404 화면보다 **나쁩니다** — 색인되므로 잘못된 URL 과 죽은 링크가 전부 검색 결과에 남습니다.
RE-008 이 정확히 그 문제를 다룹니다. **404 상태 코드가 스피너보다 중요하므로 스피너를 버렸습니다.**
로딩 표시가 필요하면 **레슨 페이지 내부에서 `<Suspense fallback=…>`** 으로 감싸세요 —
같은 피드백을 주면서 라우트 레벨 경계를 만들지 않아 소프트 404 가 생기지 않습니다.

### 404 렌더링 경로가 2가지입니다 (다음 사람이 헷갈리지 않도록)

| | 미매칭 경로 (`/x/y/z`) | 동적 라우트가 `notFound()` 호출 |
|---|---|---|
| 상태 코드 | 404 | 404 |
| `not-found.tsx` 의 title·noindex | 적용 | 적용 |
| 문구·크롬 | HTML 에 있음 | RSC 페이로드에 있음 |
| 서버 렌더 `<h1>` | **1** | **0** (React 실행 후 렌더) |

동적 라우트의 `notFound()` 가 h1 을 스트림으로 보내는 것은 **Next 의 동작이며 수정 전에도 같았습니다**
(운영 실측). 그래서 게이트는 미매칭 형태에만 서버 렌더 h1 을 요구하고, 나머지 형태에는
**상태 코드·title·noindex** 를 요구합니다 — 검색엔진이 실제로 보는 값들입니다.

### 검증 총괄

| 검증 | 결과 |
|---|---|
| `verify/verify-h1.cjs` (운영) | **23/23 PASS**, exit 0 |
| `verify/verify-error-pages.cjs` (운영) | **5/5 PASS**, exit 0 |
| `tsc --noEmit` | exit 0 |
| `next build` | `Generating static pages (1778/1778)`, TypeScript clean |
| **`content/` 변경** | **0건** ✅ |

> `next build` 는 이 환경에서 마지막 `Finalizing page optimization` 단계가 CLI safe-delete shim 에 막혀
> **exit≠0** 으로 끝납니다(5차 세션 기록과 동일). 성공 판정은 **`N/N static pages` 줄**로 하세요.
> `rm -rf .next` 도 같은 shim 에 막히므로(1342개, threshold 50) **`mv .next <저장소 밖>`** 으로 우회합니다.

---

## 1-H. 9차 세션 완료분 (2026-09-16) — RE-006 CSP

> 정본은 `NEXT-SESSION.md` §0-Z [3]. `next.config.ts` 에 이미 들어가 있던 미커밋 53줄을
> **검증까지 끝내고 커밋**했습니다. 코드는 8차 세션이 작성했고, 이 세션은 **검증·배포·기록**을 맡았습니다.

| 이슈 | 커밋 | 내용 | 검증 |
|---|---|---|---|
| RE-006 | `8c59d82` | `next.config.ts` 에 `Content-Security-Policy-Report-Only` (13 디렉티브). `CSP_REPORT_ONLY` 상수 하나로 Report-Only ↔ 강제 모드 전환 | 수정 전 운영 **0/9 exit 1** → 로컬 빌드 **11/11 exit 0** → 배포 후 운영 **11/11 exit 0** |

### 정책을 "좁게" 정한 근거는 추정이 아니라 실측입니다

`'self'` 만으로 충분한지, `'unsafe-inline'` 이 정말 필요한지를 **서빙된 HTML 로** 확인했습니다.

| 측정 | 값 | 의미 |
|---|---|---|
| `src`/`href` 안의 제3자 origin (9개 라우트) | **0** | 정책을 좁혀도 막히는 자원이 없음 |
| 외부 폰트 호스트 | **0** | `font-src 'self'` 로 충분 (`next/font` 자체 호스팅) |
| 인라인 `<script>` | 라우트당 **3~5개** | `'unsafe-inline'` 이 **필요**함 (Next 부트스트랩 + RSC flight) |
| 인라인 `style=` 속성 | 라우트당 **0~37개** | style 도 `'unsafe-inline'` 필요 |
| 무료 음원 `/audio/ld/d001.mp3` | **200** | `media-src 'self'` 유효 (라우트 핸들러가 서빙) |
| 잠긴 음원 `/audio/ld/d276.mp3` | **403** | 미디어 게이트 회귀 없음 |

즉 `'unsafe-inline'` 은 관성으로 남긴 것이 아니라 **빼면 사이트가 죽습니다.**
빼려면 요청별 nonce 가 필요하고, 그건 proxy 가 있어야 합니다(RE-016 과 같은 메커니즘).

### 신규 프로브 — `scripts/verify/verify-csp.cjs`

`node docs/qa-2026-09-15/scripts/verify/verify-csp.cjs <base>` → `11/11 routes pass`, exit 0.
`out/verify-csp.json` 에 결과를 씁니다.

**검사 축이 2개입니다** (RE-016 의 교훈: 경로만 늘리고 축을 안 늘리면 통과는 증거가 아님):

1. **정책이 우리가 쓴 것인가** — 13개 디렉티브를 **문자열 그대로** 대조. 오타·누락을 잡습니다.
2. **정책이 페이지가 실제로 쓰는 것을 허용하는가** — 서빙된 HTML 에서 `src`/`href` 의 절대 origin 을
   모아 `'self'` 와 대조. **정책이 형식상 완벽해도 틀릴 수 있는 유일한 지점**이고, 이 축만이 잡습니다.

추가로 **강제 헤더(`Content-Security-Policy`)가 없어야** 통과합니다. 모드 전환이 조용히 일어나지 않게요.

### 프로브 자체에서 한 번 헛짚었습니다 (기록)

첫 로컬 실행이 **8/9 FAIL** 이었습니다 — `k-ig-core.vercel.app` 이 `src/href` 에 있어 제3자로 판정됐습니다.
**`metadataBase` 때문에 canonical·`og:url` 이 항상 운영 도메인**이라, 로컬 서버에서는 자기 자신을
제3자로 봅니다. `verify-metadata.cjs` 가 origin vs 경로로 같은 실수를 했던 그 함정입니다.

→ 수정: self origin 을 **하드코딩하지 않고 페이지에서 읽습니다**(canonical·`og:url` + 요청 호스트).
로컬·preview·운영 어디서나 맞고, 드리프트할 상수가 없습니다. 추론한 self origin 을 **출력**하므로
canonical 이 엉뚱한 곳을 가리키면 allow-list 가 조용히 넓어지는 대신 눈에 보입니다.
...
|---|---|
| `./node_modules/.bin/tsc --noEmit` | exit 0 |
| `verify/verify-csp.cjs` (로컬 `next start -p 3210`) | **11/11 PASS**, exit 0 |
| `verify/verify-csp.cjs` (운영, 수정 전) | **0/9 PASS**, exit 1 — 전 라우트 헤더 없음 |
| `verify/verify-csp.cjs` (운영, 배포 후) | **11/11 PASS**, exit 0 |
| 운영 헤더 실측 | Report-Only 308자/13 디렉티브, 강제 헤더 **0건**, 나머지 5종 유지 |
| `next build` | 같은 워킹트리에서 `1778/1778` (routes-manifest 에 정책 포함 확인) |
| **`content/` 변경** | **0건** ✅ |

### 같은 배포에서 확인한 것 — RE-016 404 백지 후속 (`9e65501`)

제 커밋 바로 앞 커밋이고, 같은 프로덕션 배포에 실렸습니다. **운영 실측**(`<script>` 제거 후 텍스트):

| 경로 | 수정 전 | 배포 후 |
|---|---|---|
| `/ld/nope` · `/x/y` · `/t/nope` | 404 + **0자** (백지) | 404 + **744자** |
| `/ld/d001` | 200 | 200 (**960자**, 회귀 없음) |

### 환경 함정 추가 (다음 세션 시간 절약)

- **동시 작성자가 이번에도 있었습니다.** 세션 시작 시점(06:23)에 다른 세션이 `next build` 중이었고
  (`.next` 에 06:25:44 까지 파일 기록), 06:38 에 재빌드, 06:42 까지 프로브 실행, 그 사이 `9e65501` 커밋.
  → **git 작업 전에 조용한지 확인**하고, 커밋은 **파일을 이름으로 지정**해 했습니다.
- `find -printf '%TH:%TM'` 로 정렬하면 **문자열 정렬이라 "23:54" 가 "06:35" 보다 커집니다.** 신선도를
  판정할 때는 **epoch(`%T@`)로 비교**하세요. 이걸로 "아직 활동 중"을 한 번 잘못 읽었습니다.
- 로컬 검증은 **포트를 겹치지 않게** 잡으세요. 다른 세션이 3100 을 쓰고 있어 **3210** 을 썼습니다.
- `next start` 는 **기존 빌드만으로도** 동작합니다. `next.config.ts` 의 `headers()` 는 빌드 산출물
  (`.next/routes-manifest.json`)에 들어가므로, 재빌드 없이 `next start` 로 헤더를 실제로 확인할 수 있습니다.
- `.next` 를 `mv` 하려 했으나 **Permission denied**(다른 세션의 서버가 잡고 있었음) → 빌드 중에는
  `.next` 를 옮기지 마세요.
...
### ✅ 배포 결과 — 커밋 `173196d`, 반영 약 50초, **퍼지 불필요**

| 확인 (운영) | 결과 |
|---|---|
| 캐시 우회(`?cb=`) 폐지 6개 폴더 표본 | **12/12 = 403** + `private, no-store` |
| 일반 URL 이 `x-vercel-cache: HIT` 로 200 인 건수 | **0/12** — 전부 403. **Vercel 퍼지 불필요** (수정 전 HIT 였던 6개 포함) |
| 무료 레슨 트랙 음원 | **10/10 = 200 + `Content-Length > 0`** (`s1-1-1` 163,464 · `s1-1-9` 33,777 · `s1-2-1` 288,981 · `s1-2-9` 52,125 · `gh1-006-1` 1,561,495 …) |
| 유료 음원 | **5/5 = 403** (`ld/d276` · `reading/pr100` · `student/s1-3-1` · `grammar1/gh1-020` · `video/cnn/cnn003`) |
| **`verify-media-access.cjs` (운영)** | **24/24 PASS, exit 0** (수정 전 16/24 exit 1) |
| 음성 출처 문구 (운영 8개 라우트) | **0건** — `/student` 에 `▶ 무료 파트 연속 듣기` / `1·2강 연속 재생` 확인 |

> 엣지 캐시가 저절로 사라진 이유는 **새 배포가 새 캐시 네임스페이스를 갖기 때문**으로 보입니다.
> 소유자 승인 후 `FREE_PREVIEW_LESSON_IDS.grammar1` 에 `-1`·`-2` 를 **명시** 추가했습니다
> (정규화 규칙은 미디어 트랙용으로 유지). 소유자가 레슨 3,032개·미디어 참조 4,186개를 전수로 확인 —
> 새로 열리는 레슨 8 · 미디어 24 · **유료 확대 0건**.
...
### 운영 확인 — 그리고 이 변경은 운영에서 관측되지 않는다

| 확인 (운영) | 결과 |
|---|---|
| `verify-quiz-flag.cjs` (플래그 OFF 기대) | **3/3 PASS, exit 0** |
| `verify-quiz-flag.cjs --expect-on` (음성 대조) | **0/3 FAIL, exit 1** — 배포된 빌드의 플래그가 `false` 임을 확인 |

> ⚠️ **KIG-008 은 운영에서 보이는 변화가 없습니다.** 수정 전후 모두 플래그가 `false` 라
> 손님이 보는 화면이 같습니다. 그러므로 "배포가 반영됐다"를 외부에서 증명할 방법이 없고,
> **이 이슈의 증거는 로컬 양방향 측정(위 표)입니다.** 운영 프로브는 회귀 확인입니다.

---

## 1-K. 10차 세션 (계속) — RE-008 사이트맵 + 잠긴 레슨 noindex (2026-09-16)

> 정본은 `NEXT-SESSION.md` §0-Z [3]. 소유자 승인 범위: 사이트맵 정리 + `noindex`(3-5 결정).

### 실측한 결함 (운영, 비로그인, 1,751 URL 전수, `<script>` 제거 후 텍스트)

| 범주 | URL 수 | 200 + 본문 ≥300자 |
|---|---:|---:|
| 홈 | 1 | 1 |
| 과정 페이지 | 7 | 7 |
| 무료 미리보기 레슨 | 30 | 30 |
| **잠긴 레슨** | **1,713** | 75 (STUDENT 페이월 306~327자) |
| 합계 | **1,751** | 113 |

**1,713 / 1,751 = 97.8% 가 페이월 껍데기**(270자 = 내비 + `🔒 ALL-PASS ONLY` + 구매 안내)이고,
레슨마다 **제목만 다릅니다.** 사이트맵은 "색인해 달라"는 요청이므로, 거의 같은 270자 페이지 1,700개를
색인 요청하는 것은 본문이 있는 113개 페이지의 평가까지 끌어내립니다.

또한 잠긴 레슨 페이지는 **`robots` 메타를 아예 내보내지 않아**(`/ld/d003`·`/grammar1/gh1-020` 실측
`NO robots meta`) 사이트맵에서 빼도 **색인은 그대로 됩니다.** 과정 대시보드가 모든 레슨으로 링크를 걸고
있어 크롤러가 링크를 타고 들어옵니다. → 사이트맵만 고치면 절반만 닫힙니다.

### 수정

**`src/app/sitemap.ts`** — 필터를 **게이트와 같은 술어**로
```ts
const lessonPages: MetadataRoute.Sitemap = getAllLessonParams()
  .filter(({ course, lesson }) => isFreePreviewLesson(course, lesson))
  .map(/* 기존 그대로 */);
```
`isFreePreviewLessonServer`(= 게이트가 쓰는 함수)가 `isFreePreviewLesson` 에 위임하므로
**사이트맵과 게이트가 어긋날 수 없습니다.** 목록을 손으로 적으면 언젠가 조용히 드리프트합니다.
`getTabs()` 로 `/t/[tab]` 7개도 추가(실제 본문이 있는데 빠져 있었음).

**`src/app/[course]/[lesson]/page.tsx`** — 잠긴 레슨에만 `noindex`
```ts
...
```
`follow: true` 는 링크를 계속 타게 하기 위함입니다. 무료 레슨은 `robots` 키를 **아예 만들지 않습니다**
(전역 설정이 없으므로 키가 있거나 없거나 둘 중 하나이고, 상속되지 않습니다).

**`src/app/student/[lesson]/page.tsx`** — 같은 처리 + **중복 목록 제거**
이 라우트는 `const isFree = id === "s1-1" || id === "s1-2"` 라는 **리터럴 목록**을 따로 들고 있었습니다.
`FREE_PREVIEW_LESSON_IDS.student` 와 값은 같지만 **두 번째 목록**이라 한쪽만 바뀌면 게이트와
사이트맵이 어긋납니다. `isFreePreviewLesson("student", id)` 로 통일했습니다(오늘 동작 동일).

### 검증 — 신규 `verify-sitemap.cjs`, 6축

| 축 | 내용 |
|---|---|
| A | 사이트맵의 모든 URL → 200 + 본문 ≥150자 + **페이월 문구 없음** |
| B | 잠긴 레슨이 하나도 안 실렸는지 — 저장소의 `isFreePreviewLesson` 으로 교차 검사 |
| C | URL 수가 저장소가 함의하는 값과 일치(45), 중복 0 |
| D | 잠긴 레슨 표본 → `noindex, follow` 있음 |
| **D2** | **홈 · 과정 7 · 탭 7 · 무료 레슨 → `noindex` 없음** (블라스트 반경) |
| E | 탭 7개가 실렸는지 |

> ⚠️ **A 의 판정을 길이로 할 수 없습니다.** 잠긴 레슨이 **270자**, 탭 페이지가 **188~229자** —
> 페이월이 더 깁니다. 처음에 300자 기준으로 만들었다가 탭 7개가 걸려서 알았습니다.
> 그래서 바닥값 150자는 "백지/오류만 잡는" 용도로 내리고, 실제 판정은 **페이월의 문구**
> (`LessonPaywall.tsx:52,67` 의 `ALL-PASS ONLY`·`STUDENT PASS ONLY`·`VIP ALL-PASS REQUIRED`)로 합니다.
> 구조적 판정은 축 B(저장소에서 파생)가 맡고, A 의 문구 검사는 그 보강입니다.

| 검증 | 결과 |
|---|---|
| **`verify-sitemap.cjs` (운영, 수정 전)** | **1/6 FAIL, exit 1** — A 1,713건 · B 1,713건 · C 1,751≠45 · E 0/7 |
| **`verify-sitemap.cjs` (로컬, 수정 후)** | **6/6 PASS, exit 0** — 45/45 본문 · 잠긴 레슨 0건 · 45개 중복 0 · `noindex, follow` · D2 통과 |
| `tsc --noEmit` | exit 0 |
| `next build` | `✓ Generating static pages using 7 workers (1779/1779)`, exit 0 |
...
① 지문 앞 2~3문장 + 어휘 목록을 보여주고 ② 유료 콘텐츠 구조화 데이터
(`isAccessibleForFree: false`)를 붙이면 **색인 가치와 구매 동선을 둘 다** 얻습니다.
`noindex` 는 지금 상태에서 손실을 막는 차선책입니다.

⚠️ 다만 **KIG-001(잠금 레슨 본문 유출)과 정면으로 맞닿아 있습니다** — 무엇을 보여줄지와
RSC 페이로드에 무엇이 실릴지를 함께 설계해야 합니다. 별도 이슈로 다루세요.
...
> 10차: `RE-004` 미디어 허용 목록 + `KIG-009` 잔여 11곳 + STUDENT 무료 트랙 음원(§1-I) · `KIG-008` 플래그화(§1-J) · `RE-008` 사이트맵+noindex(§1-K)
> 위 33건은 §1·§1-B·§1-C·§1-D·§1-F·§1-G·§1-H·§1-I·§1-J·§1-K에 검증 결과까지 기록되어 있습니다. 다시 손대지 마세요.
...
> | 6 | ~~RE-008~~ | 사이트맵 잠긴 레슨 제외 + 잠긴 레슨 noindex | ✅ **10차 완료** (§1-K) |
...
> STATUS-2026-09-16 §3-3) 서버에는 남지 않으므로, **사람이 실제 브라우저로 각 라우트를 열어**
> 콘솔을 봐야 합니다. 자동 프로브는 "정책이 페이지가 쓰는 자원을 허용하는가"까지만 증명합니다.
>
