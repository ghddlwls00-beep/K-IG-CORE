...
| KIG-024 (P2) | (아래 커밋) | 타이핑 딕테이션이 공백 변형을 오답 처리하던 문제. 비교 전 양쪽 공백 정규화 (탭·붙여넣기 개행도 단어 구분자로 처리) | 실제 문장 277개 × 공백 변형 1,108건 → 수정 후 **1,108/1,108 정답**. 수정 전엔 833건이 오답 처리. **오답이 정답으로 통과한 건 0건** |
