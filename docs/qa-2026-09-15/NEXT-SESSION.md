...
> **지금 지시는 [`docs/qa-2026-09-18/지금-할-일.md`](../qa-2026-09-18/지금-할-일.md) 에 있습니다.** 이 문서는 지난 기록입니다.
...
> **2026-09-18: 2026-09-17 감사의 수정은 전부 배포됐고 (`d6da0a4` … `614b696`, 항목별 커밋은 [`../qa-2026-09-17/README.md`](../qa-2026-09-17/README.md) "다음 단계" 3번), 서버 함수는 서울(`icn1`, `vercel.json`)에서 돕니다.**
> 🔴 **감사 판정(NO-GO, 49/100)은 아직 갱신 전입니다.** 운영 전체 재점검은 소유자의 전체 검수 명령어로 새 세션에서 합니다. 소유자·변호사 대기 항목은 같은 README 에. 전체 스테이징 금지.
...
### 할 일 — 🔴 감사 결과 수정 대기 (2026-09-17)

**🔴 전체 출시 감사 완료 → 수정은 소유자 확인 후.** 유료 포함 전 레슨 내용·화면·음성·잠금·성능·보안을 전수 점검했습니다
(`docs/qa-2026-09-17/`). **Critical 1: READING 유료 지문 254개 전부가 공개 JS 청크에 들어 있음** (`ReadingLearningView.tsx:13-14` →
`readingUtils.ts` 중앙 사본). High 44건(이슈 16개). 우선순위는 `final-report.md` §20, 재시험은 §19.
이전에 적었던 "READING 스크립트 페이지 카드 510장" 은 R-00 으로 등록됨. **(2026-09-18 갱신) 소유자·변호사 대기 항목 외에는 모두 고쳐서 배포함 — 운영 전체 재점검 전.**

**✅ 배포 완료 — 외부 출시 감사(37건) 대응 (`c14c03d`).** 이슈 레지스터·실측값은
...
- 커밋은 원격 `62af8bb` 위로 올렸고 소유자가 푸시해 배포됨. 운영 재측정: 하네스 43 PASS / 0 FAIL / 7 INFO. 추적 클립 2개는 저장소에서 제거(R2 에 동일 파일 있음).
...
### 🔴 서버 지역은 `vercel.json` 이 정합니다 (PERF-04, 2026-09-18)

`"regions": ["icn1"]` (서울). Vercel 대시보드의 Function Region 칸은 이 파일에 덮여 **무시됩니다**.
되돌리려면 이 파일을 지우고 푸시하세요. **Vercel 의 Instant Rollback 은 쓰지 마세요** — 그 뒤의 푸시가
운영에 자동 반영되지 않게 꺼집니다. 확인: 응답 헤더 `x-vercel-id` 가 `icn1::icn1::…`,
`node docs/qa-2026-09-17/scripts/measure-latency.cjs --label <이름>`.
...
> | A-4 `isEnglish()` | ✅ **화면 틀어짐 0** — `GrammarLearningView.tsx:183-199` 보정이 받음. `docs/qa-2026-09-18/scripts/check-grammar-sides.cjs`: 앱 규칙 0 · 대조군(보정 없는 단순 규칙) 36(양방향 중복 포함, 2026-09-24 다시 잼 — 2026-09-23 기록은 21) | — |
