...
> **`main` 은 `5d94ad1` 로 푸시되어 있고, 작업 트리는 깨끗합니다.** 반쯤 된 변경은 없습니다.
...
**[1] RE-005 — VOCA 괄호가 음성으로 읽힘 (승인)**

`colo(u)r` · `gray(grey)` 등 14건. **표시용 문자열은 그대로 두고, 합성에 넘기는
문자열에서만 괄호를 푸는 방향으로 하세요.** 화면에는 `colo(u)r` 이 그대로 남아야
철자 변이를 가르치는 의미가 살고, 음성은 `color` 하나만 읽습니다.

⚠️ 합성 입력이 바뀌면 **클립 키(텍스트 해시)가 바뀌므로 그 14건은 새 클립이
