...
## 0-Z. 🔴 지금 작업 대기열 — 새 세션은 여기부터 (2026-09-16 05:50 기준)

> 이 절이 현재 작업 지시입니다. 아래 §A·§B 는 지난 세션의 기록입니다.

### 먼저 확인할 것 — `next.config.ts` 가 미커밋 상태입니다

직전 세션이 **RE-006 CSP 를 작업하다 서비스 오류로 끊겼습니다.** `next.config.ts` 에
`Content-Security-Policy-Report-Only` 53줄이 들어가 있고 커밋되지 않았습니다.
내용을 검토한 뒤 이어서 끝내거나, 판단이 서지 않으면 되돌리고 [3]에서 다시 하세요.
**끊긴 작업이니 남의 것으로 취급하지 말고 당신이 처리하면 됩니다.**

### 순서

**[0] 커밋 `98c740e` 의 후속 수정 2건을 커밋하세요 (이미 고쳐져 있습니다)**

`getCollocation` 인자가 `(word, meaning, searchWord)` → `(word, searchWord)` 로 바뀌었는데
호출부 두 곳이 옛 방식이었습니다. 둘 다 `.cjs`/`.mjs` 라 `tsc` 를 통과합니다.

| 파일 | 증상 |
|---|---|
| `docs/qa-2026-09-15/scripts/audit-data.cjs:239` | `Cannot read properties of null` — 저장소 전체 데이터 감사 도구가 죽었습니다 |
| `scripts/generate-azure-ava.mjs:145` | 한국어 `meaning` 이 조회 키가 되어 전부 null. **에러 없이 0개 수집** — 음성 재생성 때 연어 클립이 통째로 빠집니다 |

수정 후 확인된 값: `audit-data` → `speech texts 15631 · original audio srcs 2512`,
`generate-azure-ava --dry-run` → `pending 0`, `tsc --noEmit` → 오류 0.

```bash
git add docs/qa-2026-09-15/scripts/audit-data.cjs scripts/generate-azure-ava.mjs
```

> 교훈: TS 함수 시그니처를 바꿀 때 `.cjs`/`.mjs` 호출부를 grep 하세요. 타입 검사가 안 잡습니다.
> 그리고 두 번째처럼 **에러 없이 틀린 결과**가 더 위험합니다.

**[1] RE-016 404 백지 — `proxy` 로 진행 (승인됨). 상세는 바로 아래 §0-A**
...
**[3] RE-006 CSP** — 위의 미커밋 변경을 마무리. Report-Only 로 먼저 배포하고
운영 콘솔 위반 0건을 확인한 뒤에 강제 모드로.

**[4] RE-004 🔴** `src/lib/mediaAccess.ts` 의 `unclaimed → allowed:true` 기본값 때문에
폐지 과정 음원 1,463개가 인증 없이 받아집니다. 허용 목록 방식으로.
⚠️ 좁히다가 현행 과정 음원을 막으면 유료 회원 음성이 끊깁니다.
완료 조건: 폐지 과정 음원 표본 403 / 무료 레슨·Ava 클립 200 유지 / `/api/media-health` 정상.

**[5] RE-008** 사이트맵에서 잠긴 레슨 약 1,700 URL 제외.
완료 조건: 사이트맵의 모든 URL 이 비로그인 200 + 본문 있음.

**[6] RE-005** VOCA `colo(u)r` / `gray(grey)` 등 14건이 괄호째 TTS 로 넘어갑니다.
표시용/합성용 문자열 분리. ⚠️ **텍스트가 바뀌면 음성 클립 재생성 필수**(클립 키가 텍스트 해시).
...
- 프로브의 **검사 축**을 늘리세요. 경로 수만 늘리면 같은 실수가 반복됩니다 (RE-016 전례).
- 커밋 전 반드시 `npx tsc --noEmit` + `npx next build`.
- 각 건마다 운영 실측값을 숫자로 보고하세요.

### 🔴 절대 하지 말 것

- **`git add -A` / `git add .` 금지.** 파일을 이름으로 지정해서만 add 하세요.
  아래는 **다른 세션 작업물**이니 건드리지도 커밋하지도 마세요:
  ```
  docs/qa-2026-09-15/NEXT-SESSION.md
  docs/qa-2026-09-15/PROMPT.md
  docs/qa-2026-09-15/kig006-decisions.json
  docs/qa-2026-09-15/scripts/review-kig006.cjs
  docs/qa-2026-09-15/scripts/apply-kig006.cjs
  docs/qa-2026-09-15/scripts/archive-probe.cjs
  docs/qa-2026-09-15/scripts/compare-archive.cjs
  docs/qa-2026-09-15/evidence/kig006-review-table.md
  scripts/backup-license-data.mjs
  ```
- **`content/` 에 쓰지 마세요.** 원본 아카이브 대조 전까지 금지입니다.
- **`apply-kig006.cjs --write` 실행 금지.** 소유자가 아카이브 대조 이후로 정했습니다.
- `git stash` 금지 (이 저장소에서 `fatal: unable to read tree` 이력).

---

## 0-A. RE-016 남은 백지 — 시도해서 실패한 두 가지 (반복하지 마세요)
